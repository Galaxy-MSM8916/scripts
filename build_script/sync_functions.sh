...
function sync_manifests {
    ...
    manifest_url="https://git.${soc}.com/Galaxy-${soc^^}/local_manifests.git/plain"
    ...
    if [ `hostname` == "msm8916.com" ]; then
        sed -i s/fetch=\"https:\\/\\/github.com\"/fetch=\"ssh:\\/\\/jenkins@review.${soc}.com:${gerrit_port}\"/g ${local_manifest}
    fi
...
if [ -n "$SYNC_ALL" ]; then
    ...
    case $ver in
        ...
        15* | oreo )
            ...
            wget https://raw.githubusercontent.com/Galaxy-${soc^^}/repopicks/master/repopicks-${ver}.sh -O $REPOPICK_FILE
            if [ "$?" -eq 0 ]; then
...
function apply_repopicks {
    ...
    for pick in $LOCAL_REPO_PICKS; do
        $REPOPICK -g $gerrit_url -r $pick
    done
    ...
    for pick in $LINEAGE_REPO_PICKS; do
       $REPOPICK -r $pick
    done
