...
function upload_artifacts {
    ...
    if [ "$BUILD_TARGET" == "recoveryimage" ]; then
        create_github_release ${rec_name}
        upload_github_artifacts ${rec_name} ${ARTIFACT_OUT_DIR}
    elif [ "$BUILD_TARGET" == "bootimage" ]; then
        create_github_release ${bimg_name}
        upload_github_artifacts ${bimg_name} ${ARTIFACT_OUT_DIR}
    elif [ "$BUILD_TARGET" == "otapackage" ]; then
        create_github_release ${arc_name}
        upload_github_artifacts ${arc_name} ${ARTIFACT_OUT_DIR}
    fi
